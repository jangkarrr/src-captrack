...
include '../assets/includes/notification_functions.php'; // ADD THIS LINE
...
// Check login and faculty access via multi-role session
if (!isset($_SESSION['user_id']) || !isset($_SESSION['user_data'])) {
    header("Location: ../users/login.php");
    exit();
}
if (!hasRole($_SESSION['user_data'], 'faculty')) {
    header("Location: ../users/login.php?error=unauthorized_access");
    exit();
}

// Ensure active role is faculty
...
$notification = '';
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $project['faculty_approval'] !== 'approved') {
    ...
    if ($checkApprovalResult->num_rows > 0) {
        ...
        $approvalQuery = $conn->prepare("
            ...
            SET faculty_approval = ?, faculty_comments = ? 
            WHERE project_id = ?
        ");
    ...
    if ($approvalQuery->execute()) {
        ...
        $ownerQuery = "SELECT sd.$sd_pk as user_id 
                       FROM project_working_titles pw 
                       INNER JOIN student_details sd ON pw.submitted_by = sd.email 
                       WHERE pw.id = ?";
        ...
        if ($ownerData = $ownerResult->fetch_assoc()) {
            ...
            if ($approvalStatus === 'approved') {
                createNotification(
                    $conn, 
                    $user_id, 
                    'Project Approved by Faculty', 
                    'Your project "' . htmlspecialchars($project['project_title']) . '" has been approved by the faculty advisor.',
                    'success',
                    $projectID,
                    'project'
                );
                $notification = 'Project approved and forwarded to the adviser.';
            } elseif ($approvalStatus === 'rejected') {
                createNotification(
                    $conn, 
                    $user_id, 
                    'Project Requires Revision', 
                    'Your project "' . htmlspecialchars($project['project_title']) . '" requires revision. Please check the faculty comments.',
                    'warning',
                    $projectID,
                    'project'
                );
                ...
                createNotification(
                    $conn, 
                    $user_id, 
                    'Project Under Review by Faculty', 
                    'Your project "' . htmlspecialchars($project['project_title']) . '" is being reviewed by the faculty advisor. Please wait for feedback.',
                    'info',
                    $projectID,
                    'project'
                );
...
<div class="main-content">
    ...
    <a href="review_project.php" class="back-button-creative">
        ...
        <?php if ($project['faculty_approval'] === 'approved'): ?>
            <p><strong>Status:</strong> <span style="color: green; font-weight: bold;">Approved</span></p>
            <p><strong>Comments:</strong> <?php echo nl2br(htmlspecialchars($project['faculty_comments'])); ?></p>
        <?php else: ?>
            <p><strong>Status:</strong> <span style="color: <?php echo $project['faculty_approval'] === 'rejected' ? 'red' : 'orange'; ?>; font-weight: bold;">
                <?php echo ucfirst(isset($project['faculty_approval']) ? $project['faculty_approval'] : 'Pending'); ?>
            </span></p>
            <form method="POST">
                <label for="approval_status" class="form-label">Approval Status:</label>
                <select name="approval_status" id="approval_status" class="approval-status" required>
                    <option value="" disabled <?php echo !$project['faculty_approval'] ? 'selected' : ''; ?>>Select an option</option>
                    <option value="pending" <?php echo $project['faculty_approval'] === 'pending' ? 'selected' : ''; ?>>Re-Pending with Comment</option>
                    <option value="approved" <?php echo $project['faculty_approval'] === 'approved' ? 'selected' : ''; ?>>Approve</option>
                    <option value="rejected" <?php echo $project['faculty_approval'] === 'rejected' ? 'selected' : ''; ?>>Reject</option>
                </select>
                <br><br>
                <label for="comments" class="form-label">Comments:</label>
                <textarea name="comments" id="comments" class="comments-textarea" placeholder="Add your comments here..."><?php echo htmlspecialchars(isset($project['faculty_comments']) ? $project['faculty_comments'] : ''); ?></textarea>
                <br><br>
