...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($last_name) || empty($first_name) || empty($email) || empty($password) || empty($confirm_password) || empty($gender) || empty($year_section)) {
        ...
        // Check if email already exists in employees, student_details or pending_users
        $stmt = $conn->prepare("SELECT id FROM employees WHERE email = ? UNION SELECT id FROM student_details WHERE email = ? UNION SELECT id FROM pending_users WHERE email = ?");
        if (!$stmt) {
