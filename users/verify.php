...
function verifyUser($conn, $code, &$error_message) {
    ...
    try {
        ...
        // Insert into student_details table instead of users
        $stmt = $conn->prepare("INSERT INTO student_details (last_name, first_name, middle_name, email, password, gender, role, year_section, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $status = 'pending'; // Students need admin verification
        $stmt->bind_param("sssssssss", $user['last_name'], $user['first_name'], $user['middle_name'], $user['email'], $user['password'], $user['gender'], $user['role'], $user['year_section'], $status);
        if (!$stmt->execute()) {
            $error_message = "Error inserting into student_details: " . $conn->error;
            $conn->rollback();
            return false;
        }
        ...
        $admin_query = "SELECT id FROM employees WHERE role = 'admin' LIMIT 1";
