...
if ($hasRolesColumn) {
    ...
    $facultyQuery = $conn->prepare("SELECT $emp_pk AS id, $emp_first_col AS first_name, $emp_last_col AS last_name FROM employees WHERE role = 'faculty' ORDER BY $emp_first_col, $emp_last_col");
} else {
    // No role column, return empty result
    $facultyQuery = $conn->prepare("SELECT $emp_pk AS id, $emp_first_col AS first_name, $emp_last_col AS last_name FROM employees WHERE 1=0");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['assign_adviser'])) {
    ...
    $facultyNameQuery = $conn->prepare("SELECT $emp_first_col AS first_name, $emp_last_col AS last_name FROM employees WHERE $emp_pk = ?");
    ...
    if ($updateQuery->execute()) {
        ...
        $studentQuery = $conn->prepare("SELECT $sd_pk AS id FROM student_details WHERE email = ?");
        ...
        if ($studentData) {
            createNotification(
                ...
                'A books adviser (' . htmlspecialchars($facultyName) . ') has been assigned to your project "' . htmlspecialchars($projectData['project_title']) . '".',
                'info',
                $projectId,
                'project'
            );
        ...
        createNotification(
            ...
            'You have been assigned as books adviser for the project "' . htmlspecialchars($projectData['project_title']) . '".',
            'info',
            $projectId,
            'project'
        );
        ...
        $notification = 'Error assigning books adviser: ' . $conn->error;
    }
}
...
$recentlyAssignedQuery = $conn->prepare("
    SELECT 
        ...
        pwt.proponent_1,
        pwt.date_created,
        CONCAT(e.$emp_first_col, ' ', e.$emp_last_col) as adviser_name
    FROM project_working_titles pwt
    JOIN employees e ON pwt.noted_by = e.$emp_pk
...
<div class="main-content">
    ...
    <div class="page-header">
        <div class="container">
            ...
            <p class="page-subtitle">Assign books advisers to student research projects</p>
        </div>
    </div>
                ...
                <?php else: ?>
                    <div class="assignment-card">
                        <div class="empty-state">
                            ...
                            <h4>All Caught Up!</h4>
                            <p>All submitted projects have been assigned books advisers.</p>
                        </div>
                    </div>
                <?php endif; ?>
            </div>
...
document.querySelectorAll('form').forEach(form => {
    form.addEventListener('submit', function(e) {
        ...
        if (!select.value) {
            e.preventDefault();
            alert('Please select a faculty member to assign as books adviser.');
            select.focus();
        }
    });
});
