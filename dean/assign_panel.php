...
// Detect column names for employees table (needed in POST handling)
$emp_columns = [];
    ...
    if (isset($_POST['action'])) {
        if ($_POST['action'] === 'assign') {
            ...
            if ($checkResult->num_rows > 0) {
                ...
                if ($insertQuery->execute()) {
                    ...
                    $studentsQuery = $conn->prepare("
                        SELECT DISTINCT sd.$sd_pk as user_id 
                    ...
                    // Get panelist name
                    $panelistQuery = $conn->prepare("SELECT $emp_first_col AS first_name, $emp_last_col AS last_name FROM employees WHERE $emp_pk = ?");
            ...
            $assignmentQuery = $conn->prepare("
                SELECT pa.group_code, e.$emp_first_col AS first_name, e.$emp_last_col AS last_name 
                FROM panel_assignments pa
                INNER JOIN employees e ON pa.panelist_id = e.$emp_pk
                WHERE pa.id = ?
            ");
...
if ($hasRolesColumn) {
    ...
    $panelistsSql = "SELECT $emp_pk AS id, $emp_first_col AS first_name, $emp_last_col AS last_name, email FROM employees WHERE LOWER(COALESCE(role,'')) = 'panelist' OR FIND_IN_SET('panelist', REPLACE(LOWER(COALESCE(roles,'')), ' ', '')) > 0 ORDER BY $emp_first_col, $emp_last_col";
} elseif ($hasRoleColumn) {
    // Fallback for schema without roles column
    $panelistsSql = "SELECT $emp_pk AS id, $emp_first_col AS first_name, $emp_last_col AS last_name, email FROM employees WHERE LOWER(role) = 'panelist' ORDER BY $emp_first_col, $emp_last_col";
} else {
    // No role column, return empty result
    $panelistsSql = "SELECT $emp_pk AS id, $emp_first_col AS first_name, $emp_last_col AS last_name, email FROM employees WHERE 1=0";
}
...
// Get all unique group codes from student_details (not students table)
$groupsQuery = $conn->prepare("SELECT DISTINCT group_code FROM student_details WHERE group_code IS NOT NULL AND group_code != '' ORDER BY group_code");
$groupsQuery->execute();
...
$assignmentsQuery = $conn->prepare("
    SELECT pa.id, pa.group_code, pa.assigned_date, 
           e.$emp_first_col AS first_name, e.$emp_last_col AS last_name, e.email 
    FROM panel_assignments pa 
    INNER JOIN employees e ON pa.panelist_id = e.$emp_pk 
    WHERE pa.status = 'active' 
    ORDER BY pa.group_code, e.$emp_first_col, e.$emp_last_col
");
