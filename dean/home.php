...
$deanQuery = $conn->prepare("SELECT $emp_pk AS id, $emp_first_col AS first_name, $emp_last_col AS last_name FROM employees WHERE $emp_pk = ?");
...
if ($hasRolesColumn) {
    $facultyQuery = $conn->prepare("SELECT COUNT(*) as total_faculty FROM employees WHERE roles LIKE '%faculty%' OR roles LIKE '%adviser%'");
} elseif ($hasRoleColumn) {
    $facultyQuery = $conn->prepare("SELECT COUNT(*) as total_faculty FROM employees WHERE role = 'faculty' OR role = 'adviser'");
} else {
    $facultyQuery = $conn->prepare("SELECT COUNT(*) as total_faculty FROM employees WHERE 1=0"); // No faculty if no role column
}
$facultyQuery->execute();
$stats['total_faculty'] = $facultyQuery->get_result()->fetch_assoc()['total_faculty'];

// Total Active Students
// Check if students table has status column
$students_columns = [];
...
if (in_array('status', $students_columns)) {
    $studentsQuery = $conn->prepare("SELECT COUNT(*) as total_students FROM students WHERE status = 'verified'");
} else {
    // Count all students if no status column
    $studentsQuery = $conn->prepare("SELECT COUNT(*) as total_students FROM students");
}
...
// Get year_section from group_code in student_details (first 2 characters)
$yearSectionQuery = $conn->prepare("
    SELECT SUBSTRING(sd.group_code, 1, 2) AS year_section, COUNT(*) as count
    FROM project_working_titles pwt
    JOIN student_details sd ON pwt.submitted_by = sd.email
    WHERE sd.group_code IS NOT NULL AND sd.group_code != ''
    GROUP BY SUBSTRING(sd.group_code, 1, 2)
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="d-flex align-items-center p-3 bg-light rounded">
                                ...
                                <div>
                                    <h5 class="mb-1">Student Revisions</h5>
                                    <p class="text-muted mb-0">Review and approve student defense revisions</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6 d-flex align-items-center">
                            <a href="revision_management.php" class="btn btn-primary btn-lg w-100">
