...
while ($row = $result->fetch_assoc()) {
    ...
    if ($linkedUserId <= 0 && !empty($email)) {
        // Check both employees and student_details
        if ($stmtFind = $conn->prepare("SELECT id FROM employees WHERE email = ? UNION SELECT id FROM student_details WHERE email = ? LIMIT 1")) {
            $stmtFind->bind_param('ss', $email, $email);
            if ($stmtFind->execute()) {
    ...
    try {
        ...
        // Detach books foreign key first to avoid FK block on users
        if ($linkedUserId > 0) {
            if ($stmtDetach = $conn->prepare("UPDATE books SET user_id = NULL WHERE user_id = ?")) {
                $stmtDetach->bind_param('i', $linkedUserId);
                $stmtDetach->execute();
                $stmtDetach->close();
                @file_put_contents($traceFile, date('c') . " cleanup: detached books for user_id {$linkedUserId}\n", FILE_APPEND);
            } else {
                @file_put_contents($traceFile, date('c') . " cleanup: failed to prepare detach books: " . $conn->error . "\n", FILE_APPEND);
            }
        }
        ...
        if ($linkedUserId > 0) {
            // Delete from student_details (students are in student_details)
            if ($stmtDelUser = $conn->prepare("DELETE FROM student_details WHERE id = ?")) {
            ...
            // Delete from student_details by email
            if ($stmtDelUserEmail = $conn->prepare("DELETE FROM student_details WHERE email = ?")) {
