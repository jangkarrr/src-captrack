...
// Always join students table to get student names (if students table has name columns)
...
$query = "
    SELECT fd.id, fd.project_id, fd.final_defense_pdf, fd.status, fd.scheduled_date,
        pw.project_title, $studentNameSelect, $groupCodeField AS group_code,
        CASE 
            WHEN EXISTS (
        ...
        (SELECT COUNT(*) FROM defense_revisions dr 
         WHERE dr.project_id = fd.project_id AND dr.defense_type = 'final' AND dr.defense_id = fd.id 
    ...
    " . (($hasGroupCode || $hasGroupId || $hasStudentsNameCols) ? "LEFT JOIN students s ON sd.student_id = s.$students_pk" : "") . "
